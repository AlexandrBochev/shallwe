...
function PostCard({ post, dbUserId }: PostCardProps) {
  ...
  const handleAddComment = async () => {
    ...
    try {
      ...
      if (result?.success) {
        toast.success(POST.POST_CARD.commentPosted);
        setNewComment("");
      }
    } catch (error) {
      console.error(error);
      ...
      <CardContent className="p-4 sm:p-6">
        <div className="space-y-4">
          <div className="flex space-x-3 sm:space-x-4">
            <Link
              href={`${MAIN_LAYOUT.NAVBAR.profile.href}/${post.author.username}`}
            >
              <Avatar className="size-8 sm:w-10 sm:h-10 border">
                <AvatarImage
              ...
              {user ? (
                <div className="flex space-x-3">
                  ...
                  <div className="flex-1">
                    <Textarea
                      placeholder={POST.POST_CARD.writeComment}
                      value={newComment}
                    ...
                    <div className="flex justify-end mt-2">
                      ...
                      >
                        {isCommenting ? (
                          POST.POST_CARD.posting
                        ) : (
                          <>
                            <SendIcon className="size-4" />
                            {POST.POST_CARD.comment}
                          </>
                        )}
                      </Button>
                    </div>
                ...
                <div className="flex justify-center p-4 border rounded-lg bg-muted/50">
                  <SignInButton mode="modal">
                    <Button variant="outline" className="gap-2">
                      ...
                      {POST.POST_CARD.signInToComment}
                    </Button>
                  </SignInButton>
                </div>
              )}
